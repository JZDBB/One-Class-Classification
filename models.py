...
# =========================================================================================================
  def train(self, config):
    self.train = layers.optimize_loss(self.pre_loss, tf.train.get_or_create_global_step(),
      ...
      for idx in xrange(0, batch_idxs):
        ...
        if config.dataset == 'mnist' or 'cifar-10':
            if self.pre:
                ...
                all_loss, centerloss, recon = self.sess.run(
                    [self.train, self.center_loss, self.g_r_loss],
                    feed_dict={self.inputs: batch_images})

                counter += 1

                msg = "Epoch:[%2d][%4d/%4d]--> centerloss: %.8f, recon-loss: %.8f" % (
                    epoch, idx, batch_idxs, centerloss, recon)
                print(msg)
                logging.info(msg)
